...
object SemanticSimilarity  {
  ...
  def calcProbabilities(interactions:Interactions) = {
    ...
    val goids = interactions.filter(_.weight > 0).flatMap(i => i.protein1.goids++i.protein2.goids).toSet
    goids.filter(graph.contains).foreach(goid => graph.ancestors(goid).foreach(n => inc(n.label)))
    val n = probs(graph.roots.head.label)
    probs.foreach{case (label,c) => probs(label) = c/n}
    probs
  }

  def calcProbabilities1(interactions:Interactions) = {
    val probs = Map[String,Double]() ++= graph.map(node => (node.label, 0.0))
    ...
    def inc(interaction:Interaction) {
      val ancestors1 = ancestors(interaction.protein1.goids)
      val ancestors2 = ancestors(interaction.protein2.goids)
      val allAncestors = ancestors1 & ancestors2
      allAncestors.foreach{n => probs(n.label) += 1.0}
    }
    interactions.filter(_.weight > 0).foreach(inc)
    val n = probs(graph.roots.head.label)
    probs.foreach{case (label,c) => probs(label) = c/n}
    probs
  }
  ...
  def evaluate(measure: Measure) = {
    ...
    val results = XVal(interactions, 10, 1).map(evaluate).toSeq
